  ...
  useEffect(() => {
    ...
    scene.traverse((o) => {
      ...
      if (o.isBone) {
        ...
        if (
          (nameLower.includes('lefteye') || nameLower.includes('eye_l')) &&
        ...
        if (
          (nameLower.includes('righteye') || nameLower.includes('eye_r')) &&
      ...
      if ((o.isMesh || o.isSkinnedMesh) && o.morphTargetDictionary) {
        ...
        if (nameLower.includes('head') && !headMeshRef.current) {
          ...
          if (import.meta.env.DEV) {
            const hasEyeBlink = Object.keys(o.morphTargetDictionary).some(
  ...
  useEffect(() => {
    const updateEnhancements = () => {
      ...
      if (leftEyeBoneRef.current && rightEyeBoneRef.current && isPlaying) {
        ...
        leftEyeBoneRef.current.rotation.x = THREE.MathUtils.lerp(
          leftEyeBoneRef.current.rotation.x,
          lookY,
          0.05
        );
        leftEyeBoneRef.current.rotation.y = THREE.MathUtils.lerp(
          leftEyeBoneRef.current.rotation.y,
          lookX,
          0.05
        );
        rightEyeBoneRef.current.rotation.x = THREE.MathUtils.lerp(
          rightEyeBoneRef.current.rotation.x,
          lookY,
          0.05
        );
        rightEyeBoneRef.current.rotation.y = THREE.MathUtils.lerp(
          rightEyeBoneRef.current.rotation.y,
          lookX,
          0.05
        );
