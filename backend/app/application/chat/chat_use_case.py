...
class ChatUseCase:
    ...
    async def execute_rag_query(
        ...
        full_text = re.sub(r"^\[emotion:\w+\]\s*", "", full_text)
    ...
    async def stream_rag_query(
        ...
        async for token_chunk in self.llm.stream(history):
            if token_chunk.token:
                tok = token_chunk.token
                full_response.append(tok)
                
                if not emotion_stripped:
                    buffer += tok
                    # If buffer has a complete emotion tag, strip it and yield the rest
                    if "]" in buffer:
                        buffer = re.sub(r"^\[emotion:\w+\]\s*", "", buffer)
                        emotion_stripped = True
                        if buffer:
                            yield buffer
                    # If it's still collecting the tag and it looks like a tag, wait
                    elif buffer.startswith("["):
                        continue
                    else:
                        # Not an emotion tag, yield it
                        emotion_stripped = True
                        yield buffer
                else:
                    yield tok
