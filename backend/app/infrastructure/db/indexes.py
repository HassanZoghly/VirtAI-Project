...
- Sparse indexes used for optional unique fields (e.g. google_id)
...
async def _index_users(db: AsyncIOMotorDatabase) -> None:
    ...
    await db["users"].create_indexes([
        ...
        IndexModel(
            [("google_id", ASCENDING)],
            unique=True,
            sparse=True,  # null values are not indexed → multiple users can have null google_id
            name="google_id_unique_sparse",
        ),
    ])
