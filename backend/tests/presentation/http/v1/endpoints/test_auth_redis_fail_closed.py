...
async def test_redis_timeout_during_blacklist_validation():
    with patch("app.presentation.http.v1.dependencies.decode_auth_token", mock_decode_auth_token):
        with patch("app.infrastructure.cache.jwt_blacklist.is_blacklisted", AsyncMock(side_effect=asyncio.TimeoutError)):
...
async def test_redis_connection_error_during_blacklist():
    ...
    with patch("app.presentation.http.v1.dependencies.decode_auth_token", mock_decode_auth_token):
        with patch("app.infrastructure.cache.jwt_blacklist.is_blacklisted", AsyncMock(side_effect=redis.exceptions.ConnectionError("Offline"))):
            with pytest.raises(HTTPException) as exc_info:
                await _current_user(Request(scope={"type": "http"}), AsyncMock(), DummyCreds())
            assert exc_info.value.status_code == 401
            assert exc_info.value.detail == "Authentication service unavailable"
...
async def test_revoked_token_is_preserved():
    with patch("app.presentation.http.v1.dependencies.decode_auth_token", mock_decode_auth_token):
        with patch("app.infrastructure.cache.jwt_blacklist.is_blacklisted", AsyncMock(return_value=True)):
...
async def test_non_redis_exception_is_not_swallowed():
    with patch("app.presentation.http.v1.dependencies.decode_auth_token", mock_decode_auth_token):
        with patch("app.infrastructure.cache.jwt_blacklist.is_blacklisted", AsyncMock(side_effect=RuntimeError("Some logical bug"))):
    ...
    with patch("app.presentation.http.v1.dependencies.decode_auth_token", mock_decode_auth_token):
        with patch("app.infrastructure.cache.jwt_blacklist.is_blacklisted", AsyncMock(return_value=False)):
            with patch("app.infrastructure.cache.auth_session_cache.get_cached_auth_session", AsyncMock(return_value=None)):
                ...
                with patch("app.presentation.http.v1.dependencies.get_user_by_id", AsyncMock(return_value=user)):
                    with patch("app.infrastructure.cache.auth_session_cache.cache_auth_session", AsyncMock(side_effect=asyncio.TimeoutError)):
                        result = await _current_user(Request(scope={"type": "http"}), AsyncMock(), DummyCreds())
                        assert result == user
