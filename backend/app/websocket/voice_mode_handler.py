...
from app.services.asr.base import StreamingASRService
...
class VoiceModeHandler:
    ...
    def _check_rate_limit(self) -> None:
        ...
        while self._chunk_timestamps and current_time - self._chunk_timestamps[0] > self.rate_limit_window:
    ...
    async def handle_audio_chunk(self, pcm_bytes: bytes, is_final: bool = False) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Rate limit exceeded | session={self.session_id} | error={str(e)}"
            )
            ...
            logger.error(
                f"Chunk size exceeded | session={self.session_id} | error={str(e)}"
            )
            await self.websocket.send_json({
                "type": "error",
            ...
            logger.error(
                f"Invalid PCM audio data | session={self.session_id} | error={str(e)}"
            )
            await self.websocket.send_json({
                "type": "error",
            ...
            logger.exception(
                f"Unexpected error handling audio chunk | session={self.session_id}"
            )
            await self.websocket.send_json({
                "type": "error",
    ...
    async def process_accumulated_audio(self) -> None:
        ...
        try:
            ...
            if total_size == 0:
                logger.warning(
                    f"No audio data to process | session={self.session_id}"
                )
                self.audio_pipeline.clear_buffer()
                return
            ...
            if result.transcript.strip():
                ...
                logger.warning(
                    f"Empty transcript received | session={self.session_id}"
                )
    ...
    async def send_transcript(
        ...
        try:
            ...
            logger.exception(
                f"Failed to send transcript | session={self.session_id}"
            )
