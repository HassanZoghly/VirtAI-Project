...
import { VoiceActivityDetector, VADConfig } from '../audio/vad';
...
export function useVoiceMode(
    ...
    // Timer for detecting silence (when user stops speaking)
    const silenceTimerRef = useRef<number | null>(null);
    const lastAudioTimeRef = useRef<number>(Date.now());
    const SILENCE_THRESHOLD_MS = 1500; // 1.5 seconds of silence before finalizing

    // Batch send queue for WebSocket optimization
    const batchSendQueueRef = useRef<Array<{ audioData: Float32Array; timestamp: number; isFinal: boolean }>>([]);
    ...
    /**
     * Send batched audio chunks via WebSocket
     * 
     * Batches multiple chunks together to reduce network overhead.
     * Sends raw PCM data via binary frames.
     * 
     * Requirements:
     * - 14.2: Batch WebSocket sends every 100ms to reduce network overhead
     * - 2.4: Send raw PCM bytes via binary frames
     * 
     * @param chunks - Array of audio chunks to send
     */
    const sendBatchedChunks = useCallback((chunks: Array<{ audioData: Float32Array; timestamp: number; isFinal: boolean }>) => {
        if (!wsClient.isConnected || chunks.length === 0) {
            return;
        }

        // Send each chunk in the batch as binary frame
        for (const chunk of chunks) {
            // Convert Float32Array to Int16 PCM
            const int16Array = new Int16Array(chunk.audioData.length);
            for (let i = 0; i < chunk.audioData.length; i++) {
                const sample = Math.max(-1, Math.min(1, chunk.audioData[i]));
                int16Array[i] = sample < 0 ? sample * 0x8000 : sample * 0x7FFF;
            }

            // Send raw PCM bytes via binary frame
            wsClient.send(int16Array.buffer);
        }

        // Reset VAD if any chunk is final
    ...
    /**
     * Set up batch callback for optimized VAD
     * 
     * The OptimizedVADProcessor will call this callback every 100ms
     * with accumulated chunks, reducing WebSocket send overhead.
     */
    useEffect(() => {
        if (vadRef.current) {
            vadRef.current.onBatch(sendBatchedChunks);
        }
    }, [sendBatchedChunks]);
 ...
 * 
 * The OptimizedVADProcessor handles batching internally and will
 * call the batch callback every 100ms with accumulated chunks.
 * 
 * Requirements:
 * - 2.7: Process audio through VAD
 * - 2.4: Send raw PCM bytes via binary frames
 * - 14.1: VAD processing within 10ms (handled by OptimizedVADProcessor)
 * - 14.2: Batch WebSocket sends every 100ms (handled by OptimizedVADProcessor)
 * 
