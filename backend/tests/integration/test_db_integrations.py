...
async def test_summary_cache_insertion(mock_db_session):
    ...
    new_entry = SummaryCache(
        document_id=doc_id,
        summary_text="This is a dummy summary."
    )
    mock_db_session.add(new_entry)
    await mock_db_session.commit()
    
    # Retrieve it
    result = await mock_db_session.execute(select(SummaryCache).where(SummaryCache.document_id == doc_id))
...
async def test_diagram_cache_insertion(mock_db_session):
    ...
    result = await mock_db_session.execute(select(DiagramCache).where(DiagramCache.document_id == doc_id))
    entry = result.scalar_one_or_none()
