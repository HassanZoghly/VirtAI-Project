...
def classify_task_type(query: str, route_hint: str | None = None) -> TaskType:
    ...
    if route_lower == "reasoning" or route_lower == TaskType.EXPLANATION.value or any(kw in query_lower for kw in _EXPLANATION_KEYWORDS):
...
def detect_locale(text: str) -> Locale:
    ...
    is_arabic = any("\u0600" <= c <= "\u06FF" for c in text) or \
                "arabic" in text_lower or "عربي" in text_lower
