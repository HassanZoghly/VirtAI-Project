...
export function DiagramViewer({ diagramData, isLoading, onClose }: DiagramViewerProps) {
  ...
  return (
    <div className="w-full h-full relative flex flex-col bg-[#0A0908] overflow-hidden">
      {/* Floating Overlay Controls */}
      <div className="absolute top-4 right-4 flex items-center gap-3 z-50">
        <button
          onClick={handleDownloadPNG}
          disabled={!svgContent}
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#0A0908]/60 backdrop-blur-md border border-gold/15 text-sm font-medium text-gold-soft hover:bg-gold/5 hover:border-gold/30 hover:scale-[1.02] transition-[background-color,border-color,transform] duration-300 shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <FiDownload size={16} />
        ...
        <button
          onClick={onClose}
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#0A0908]/60 backdrop-blur-md border border-gold/15 text-sm font-medium text-gold-soft hover:bg-gold/5 hover:border-gold/30 hover:scale-[1.02] transition-[background-color,border-color,transform] duration-300 shadow-xl"
        >
          <FiX size={16} />
      ...
      <div className="flex-1 w-full h-full cursor-grab active:cursor-grabbing overflow-hidden relative">
        {(isLoading || (isRenderLoading && !error)) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0A0908] z-10 animate-fade-in">
        ...
        {error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0A0908] z-10 text-center p-6">
      ...
      {diagramData?.citations && diagramData.citations.length > 0 && (
        <div className="absolute bottom-4 left-4 right-4 bg-[#121212]/80 backdrop-blur-md border border-gold/15 rounded-2xl p-4 z-40 max-h-32 overflow-y-auto shadow-2xl">
