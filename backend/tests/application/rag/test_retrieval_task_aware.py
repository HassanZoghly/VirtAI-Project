...
async def test_retrieval_task_aware_simple_qa():
    ...
    chunks = [(DocumentChunk(id=uuid.uuid4(), document_id=uuid.uuid4(), chunk_text=f"text {i}", chunk_order=i, embedding=None), 0.9) for i in range(30)]
    ...
    result = await use_case.retrieve("test query", user_id=uuid.uuid4(), task_type=TaskType.SIMPLE_QA)
...
async def test_retrieval_task_aware_summary():
    ...
    chunks = [(DocumentChunk(id=uuid.uuid4(), document_id=uuid.uuid4(), chunk_text=f"text {i}", chunk_order=i, embedding=None), 0.9) for i in range(100)]
    vector_store.hybrid_search.return_value = chunks
